...
/** MasterResponse จาก backend — field ตรงกับ Position อยู่แล้ว (id/code/name/isActive). */
...
async function fetchPage(page: number): Promise<PagedResult<Position>> {
  const res = await adminFetch(`${BASE}?page=${page}&limit=${PAGE_LIMIT}`);
  if (!res.ok) throw new Error(`${BASE} ${res.status}`);
  return (await res.json()) as PagedResult<Position>;
}
