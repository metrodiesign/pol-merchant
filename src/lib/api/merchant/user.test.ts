...
import { buildRegisterFormData, merchantUserRegister } from "./user";
...
describe("merchantUserRegister", () => {
  it("POST multipart ไป /producer/register พร้อม credentials", async () => {
    const fd = new FormData();
    const response = new Response(null, { status: 201 });
    const fetchMock = vi.fn().mockResolvedValue(response);
    vi.stubGlobal("fetch", fetchMock);

    await expect(merchantUserRegister(fd)).resolves.toBe(response);
    expect(fetchMock).toHaveBeenCalledWith("/producer/register", {
      method: "POST",
      body: fd,
      credentials: "include",
    });
