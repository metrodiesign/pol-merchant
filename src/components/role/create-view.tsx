...
import { makeCodeFromName, makeCopyCode, validateRoleForm } from "@/lib/role/permissions";
...
/**
 * หน้าสร้างบทบาทแบบเต็มหน้า (route แยก /role/create).
...
export function RoleCreateView({ source }: RoleCreateViewProps) {
  ...
  function handleSaveConfirm() {
    ...
    router.push(
      `/role/list?toast=${encodeURIComponent(`${verb} “${input.name}” สำเร็จ`)}`,
    );
  }
  ...
  return (
    <>
      <PageHeader
        ...
        breadcrumbs={[
          { label: "Console" },
          { label: "บทบาทและสิทธิ์", href: "/role/list" },
          { label: title },
        ]}
        actions={
      ...
      <ConfirmDialog
        ...
        onConfirm={() => router.push("/role/list")}
