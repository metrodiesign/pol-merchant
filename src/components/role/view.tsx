...
export function RolesView() {
  ...
  // toast เมื่อกลับจากหน้าแก้ไขแยก (/role/edit -> /role/list?toast=...) (REQ-13.1)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const t = params.get("toast");
    if (t) {
      show(t);
      window.history.replaceState({}, "", "/role/list");
    }
  }, [show]);
  ...
  function goDuplicate(role: Role) {
    setDetailRole(null);
    router.push(`/role/create?from=${encodeURIComponent(role.code)}`);
  }

  function goRead(role: Role) {
    setDetailRole(null);
    router.push(`/role/read?code=${encodeURIComponent(role.code)}`);
  }

  function goEdit(role: Role) {
    setDetailRole(null);
    router.push(`/role/edit?code=${encodeURIComponent(role.code)}`);
  }
