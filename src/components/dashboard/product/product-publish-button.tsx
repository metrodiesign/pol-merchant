  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 rounded-lg bg-grey-500/16 px-3 py-1.5 text-base font-semibold text-grey-700 transition-colors hover:bg-grey-500/24"
      >
        {status}
        <ChevronDown
          className={cn(
          ...
          <div className="absolute right-0 top-full z-20 mt-1 w-36 overflow-hidden rounded-xl bg-white py-1 shadow-[0_0_2px_rgba(145,158,171,0.2),0_12px_24px_-4px_rgba(145,158,171,0.12)]">
            {OPTIONS.map(({ value, label, Icon }) => (
              <button
                ...
                className={cn(
                  "flex w-full items-center gap-2.5 px-3 py-2 text-base transition-colors hover:bg-[var(--action-hover)]",
                  status === value
