        ...
        {continueCourses.map((course, i) => (
          <li key={course.title} className={`flex items-center gap-4 px-6 py-3 ${i < continueCourses.length - 1 ? "border-b border-dashed border-grey-300" : ""}`}>
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-base font-semibold text-grey-800">{course.title}</p>
              <p className="text-base text-grey-500">{course.lessons}</p>
              <div className="mt-1.5 flex items-center gap-2">
                ...
                <span className="text-base font-semibold text-grey-600">{course.progress}%</span>
              </div>
            </div>
          </li>
        ))}
