...
export function CalendarAgenda({ events, weekDate, onEventClick }: CalendarAgendaProps) {
  ...
  if (groups.length === 0) {
    return (
      <div className="py-12 text-center text-lg text-grey-500">
      ...
      {groups.map((group) => (
        <div key={group.dateStr}>
          ...
          {group.events.map((ev) => (
            ...
            >
              <span className="w-28 shrink-0 text-lg text-grey-500">
                {getTimeLabel(ev)}
              </span>
              <span
                className={cn(
              ...
              />
              <span className="text-lg font-medium text-grey-800">{ev.title}</span>
            </button>
          ))}
        </div>
      ))}
