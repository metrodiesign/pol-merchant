    ...
    <WidgetCard
      ...
      action={
        <Select value={year} onValueChange={(v) => v && setYear(v)}>
          <SelectTrigger className="h-10 w-[100px] rounded-control text-base data-[size=default]:h-10">
            <SelectValue />
          </SelectTrigger>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-x-6 gap-y-2">
        {areaInstalled.totals.map((t, i) => (
          <div key={t.name} className="flex items-center gap-2">
            ...
            />
            <span className="text-base text-grey-600">{t.name}</span>
            <span className="text-base font-semibold text-grey-800">{t.value}</span>
          </div>
        ))}
      </div>
      <StackedBarChart
