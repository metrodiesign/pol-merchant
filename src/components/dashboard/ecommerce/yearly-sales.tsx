...
function ChartTooltip({ active, label, payload }: ChartTooltipProps) {
  ...
  return (
    <div className="rounded-lg bg-grey-800/90 px-3 py-2 text-base text-white shadow-z8">
    ...
    <WidgetCard
      ...
      action={
        <Select value={year} onValueChange={(v) => v && setYear(v)}>
          <SelectTrigger className="h-8 w-[88px] rounded-control text-base">
            <SelectValue />
          </SelectTrigger>
    ...
    >
      ...
      <div className="mb-2 flex flex-wrap items-center gap-x-6 gap-y-2">
        {totals.map((t) => {
          ...
          return (
            <div key={t.name} className="flex items-center gap-1.5 text-base">
              <span
