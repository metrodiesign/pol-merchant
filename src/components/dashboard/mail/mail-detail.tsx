  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-end gap-1 border-b border-grey-100 px-4 py-2">
        ...
        <span className="ml-2 text-base text-grey-400">{message.timestamp}</span>
      </div>

      {/* Subject + reply cluster */}
      <div className="flex items-start justify-between gap-3 px-4 py-4">
        <h6 className="text-base font-semibold leading-snug text-grey-800">
          {message.subject}
        </h6>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        <p className="text-base leading-relaxed text-grey-600">{message.body}</p>
      </div>
    </div>
  );
}
