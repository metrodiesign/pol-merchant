...
import { validateOfficeForm } from "@/lib/organization/office/form";
import { getOffice, updateOffice } from "@/lib/api/admin/office";
...
export function OfficeEditView({ id }: OfficeEditViewProps) {
  ...
  async function handleSave() {
    ...
    const found = validateOfficeForm({ code: unit.code, name }, "edit");
    ...
    const res = await updateOffice(id, {
      name: name.trim(),
    ...
    router.push(
      `${listHref}?toast=${encodeURIComponent(`แก้ไข${OFFICE_LABEL} "${name.trim()}" สำเร็จ`)}`,
    );
  }
  ...
  const header = (
    <EditPageHeader
      title={`แก้ไข${OFFICE_LABEL}`}
      backHref={listHref}
      breadcrumbs={[
        { label: "Console" },
        { label: OFFICE_LABEL, href: listHref },
        { label: unit?.name ?? `แก้ไข${OFFICE_LABEL}` },
      ]}
    />
  );
