...
import { createOffice } from "@/lib/api/admin/office";
...
/**
 * หน้าสร้าง office แบบเต็มหน้า — field แค่ code + name (รายการใหม่เกิดเป็น active เสมอ).
...
export function OfficeCreateView() {
  ...
  const title = `เพิ่ม${OFFICE_LABEL}`;
  const dirty = code.trim() !== "" || name.trim() !== "";

  async function handleSave() {
    if (saving) return;
    const found = validateOfficeForm({ code, name }, "create");
    ...
    router.push(
      `${listHref}?toast=${encodeURIComponent(`เพิ่ม${OFFICE_LABEL} "${name.trim()}" สำเร็จ`)}`,
    );
  }
