...
import { getOffices, deactivateOffice } from "@/lib/api/admin/office";
...
import { OfficeDetailSheet } from "./detail-sheet";

/**
 * Client container หน้า list ของ office (โมดูลอิสระ — REQ-7.2).
...
export function OfficeListView() {
  ...
  const [detailUnit, setDetailUnit] = useState<Office | null>(null);
  const [deactivateUnit, setDeactivateUnit] = useState<Office | null>(null);
  ...
  useEffect(() => {
    ...
    if (t) {
      show(t);
      window.history.replaceState({}, "", `${OFFICE_BASE_PATH}/list`);
    }
  }, [show]);

  function goRead(unit: Office) {
    setDetailUnit(null);
    router.push(`${OFFICE_BASE_PATH}/read?id=${encodeURIComponent(unit.id)}`);
  }

  function goEdit(unit: Office) {
    setDetailUnit(null);
    router.push(`${OFFICE_BASE_PATH}/edit?id=${encodeURIComponent(unit.id)}`);
  }
  ...
  const table = useDataTable<Office>({
    data: units,
    columns,
    getRowId: (u) => u.id,
    meta: { onRowClick: (unit: Office) => setDetailUnit(unit) },
    ...
    <>
      ...
      >
        ...
        {loading ? (
          ...
          <div className="px-5 py-10 text-center text-sm text-grey-500">
            <p>โหลด{OFFICE_LABEL}ไม่สำเร็จ</p>
            <button
              type="button"
              onClick={() => {
                setLoading(true);
      ...
      <OrgUnitConfirmDialog
        ...
        onConfirm={async () => {
          ...
          if (!unit) return;
          const res = await deactivateOffice(unit.id);
          if (!res.ok) {
