...
import { getLevels, deactivateLevel } from "@/lib/api/admin/level";
...
import { LevelDetailSheet } from "./detail-sheet";

/**
 * Client container หน้า list ของ level (โมดูลอิสระ — REQ-7.2).
...
export function LevelListView() {
  ...
  const [detailUnit, setDetailUnit] = useState<Level | null>(null);
  const [deactivateUnit, setDeactivateUnit] = useState<Level | null>(null);
  const [units, setUnits] = useState<Level[]>([]);
  ...
  useEffect(() => {
    ...
    if (t) {
      show(t);
      window.history.replaceState({}, "", `${LEVEL_BASE_PATH}/list`);
    }
  }, [show]);

  function goRead(unit: Level) {
    setDetailUnit(null);
    router.push(`${LEVEL_BASE_PATH}/read?id=${encodeURIComponent(unit.id)}`);
  }

  function goEdit(unit: Level) {
    setDetailUnit(null);
    router.push(`${LEVEL_BASE_PATH}/edit?id=${encodeURIComponent(unit.id)}`);
  }
  ...
  const table = useDataTable<Level>({
    data: units,
    columns,
    getRowId: (u) => u.id,
    meta: { onRowClick: (unit: Level) => setDetailUnit(unit) },
    ...
    <>
      ...
      >
        ...
        {loading ? (
          ...
          <div className="px-5 py-10 text-center text-sm text-grey-500">
            <p>โหลด{LEVEL_LABEL}ไม่สำเร็จ</p>
            <button
              type="button"
              onClick={() => {
                setLoading(true);
      ...
      <OrgUnitConfirmDialog
        ...
        onConfirm={async () => {
          ...
          if (!unit) return;
          const res = await deactivateLevel(unit.id);
          if (!res.ok) {
