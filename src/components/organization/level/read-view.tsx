...
import { getLevel } from "@/lib/api/admin/level";
...
interface LevelReadViewProps {
  /** id (Guid) ของรายการ — view โหลดเองจาก GET /api/v1/levels/{id}. */
  id: string;
}

/** หน้าดูรายละเอียด level แบบเต็มหน้า — read-only + ปุ่มไปหน้าแก้ไข. */
export function LevelReadView({ id }: LevelReadViewProps) {
  ...
  const header = (
    <EditPageHeader
      title={`ดู${LEVEL_LABEL}`}
      backHref={listHref}
      breadcrumbs={[
        { label: "Console" },
        { label: LEVEL_LABEL, href: listHref },
        { label: unit?.name ?? `ดู${LEVEL_LABEL}` },
      ]}
    />
  );
      ...
      <EditPageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Link
              href={`${LEVEL_BASE_PATH}/edit?id=${encodeURIComponent(unit.id)}`}
