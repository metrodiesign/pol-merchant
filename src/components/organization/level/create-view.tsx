...
import { createLevel } from "@/lib/api/admin/level";
...
/**
 * หน้าสร้าง level แบบเต็มหน้า — field แค่ code + name (รายการใหม่เกิดเป็น active เสมอ).
...
export function LevelCreateView() {
  ...
  const title = `เพิ่ม${LEVEL_LABEL}`;
  const dirty = code.trim() !== "" || name.trim() !== "";

  async function handleSave() {
    if (saving) return;
    const found = validateLevelForm({ code, name }, "create");
    ...
    router.push(
      `${listHref}?toast=${encodeURIComponent(`เพิ่ม${LEVEL_LABEL} "${name.trim()}" สำเร็จ`)}`,
    );
  }
      ...
      <div className="rounded-card bg-card p-6" style={cardStyle}>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <TextField
            ...
            onChange={setCode}
            error={errors.code}
            placeholder="เช่น lv1 (a-z, 0-9, _)"
