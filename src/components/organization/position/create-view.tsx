...
import { createPosition } from "@/lib/api/admin/position";
...
/**
 * หน้าสร้าง position แบบเต็มหน้า — field แค่ code + name (รายการใหม่เกิดเป็น active เสมอ).
...
export function PositionCreateView() {
  ...
  const title = `เพิ่ม${POSITION_LABEL}`;
  const dirty = code.trim() !== "" || name.trim() !== "";

  async function handleSave() {
    if (saving) return;
    const found = validatePositionForm({ code, name }, "create");
    ...
    router.push(
      `${listHref}?toast=${encodeURIComponent(`เพิ่ม${POSITION_LABEL} "${name.trim()}" สำเร็จ`)}`,
    );
  }
      ...
      <div className="rounded-card bg-card p-6" style={cardStyle}>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <TextField
            ...
            onChange={setCode}
            error={errors.code}
            placeholder="เช่น staff (a-z, 0-9, _)"
