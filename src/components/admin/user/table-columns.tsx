...
export const userColumns: ColumnDef<User>[] = [
    ...
    cell: ({ getValue }) => (
      <div className="flex flex-wrap gap-1">
        {getValue<string[]>().map((r) => (
          <span
            key={r}
            className="inline-flex h-6 items-center rounded-md bg-grey-600/8 px-1.5 text-base font-semibold text-grey-700"
          >
            {r}
          </span>
        ))}
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span
        className={`inline-flex items-center whitespace-nowrap rounded-full px-4 py-1 text-base font-semibold ${statusStyles[row.original.status]}`}
      >
