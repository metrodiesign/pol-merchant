...
function VcpBadge({ vcp }: { vcp: VcpStatus }) {
  if (vcp === "none") return <span className="text-base text-grey-500">N/A</span>;
...
export const policyColumns: ColumnDef<Policy>[] = [
  ...
  },
  {
    ...
    enableSorting: false,
    cell: ({ row }) => (
      <span className="text-base whitespace-nowrap text-foreground">
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="min-w-0 whitespace-nowrap">
          <span className="block text-base font-semibold text-primary underline underline-offset-2">
            {p.referenceNo}
          </span>
          <span className="mt-0.5 block text-base text-grey-500">
  ...
  },
  {
    ...
    enableSorting: false,
    cell: ({ row }) => (
      <span className="block min-w-0 truncate text-base text-foreground">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (!info.tooltip) {
        return <span className="text-base whitespace-nowrap text-foreground">{info.text}</span>;
      }
      return (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger
              render={
                <span className="cursor-help text-base whitespace-nowrap text-foreground underline decoration-dotted underline-offset-4" />
              }
            >
              {info.text}
            </TooltipTrigger>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (d.state === "none") {
        return <span className="text-base text-grey-500">N/A</span>;
      }
      return (
        <div className="min-w-0 whitespace-nowrap">
          <span className="block text-base text-foreground">ตัดชำระแล้ว</span>
          {d.date ? (
            <span className="block text-base text-grey-500">{formatThaiSlashDate(d.date)}</span>
          ) : null}
        </div>
      );
    },
