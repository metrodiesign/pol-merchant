...
- [x] 1. View components namespace ใหม่ — copy 6 ไฟล์ `dashboard/user/*` -> `src/components/user/*`
     ...
     แก้จุดเดียว: `user-table-columns` ลิงก์ edit `/dashboard/user/${u.id}/edit` ->
...
- [x] 2. Route module `src/app/user` (list/new/edit) — copy page+layout จาก 3 หน้า
     ...
     `/dashboard/user*` -> `/user*`; แก้ import view -> `@/components/user/*`;
...
- [x] 3. เมนู `UserManagement` — แทรก NavGroup ใหม่ใน `src/components/layout/nav-config.ts`
     ...
     Evidence: test pass — `tsx` import: `navConfig[1].subheader` = "UserManagement", `items.length` = 1, item = {title:"ผู้ใช้งาน & สิทธิ์", path:"/user/list", icon:"user", deepMatch:true}, ไม่มี children; `navConfig[2]` = Demo, Demo>User เดิม path `/dashboard/user` + 6 children ไม่เปลี่ยน; viewports 1440 OK (เมนู "ผู้ใช้งาน & สิทธิ์" ใต้ USERMANAGEMENT, active state เขียว ทำงาน); deviations live sidebar consume `minimalsNavConfig` (จาก `minimals-nav-config.ts`) ไม่ใช่ `navConfig` — design ระบุไฟล์ผิด. เพิ่ม group เดียวกันใน `minimals-nav-config.ts` (live, render จริง) + คงใน `nav-config.ts` ไว้ feed breadcrumb/search. พบตอน browser verify

- [x] 4. ยืนยัน coexist + ของเดิมไม่ถูกแตะ + build เขียว — ตรวจว่า `/dashboard/user/list`
     และ `/user/list` เข้าถึงได้พร้อมกัน, `git diff --name-only` ไม่มีไฟล์ใต้
     `src/app/dashboard/user` หรือ `src/components/dashboard/user`, type-check/build ผ่าน.
     ...
     Evidence: test pass — `npx tsc --noEmit` = No errors found; coexist: `src/app/dashboard/user/list/page.tsx` + `src/app/user/list/page.tsx` มีทั้งคู่; `git status --porcelain` แตะแค่ `nav-config.ts` + untracked `src/app/user/`, `src/components/user/` — ไม่มีไฟล์ใต้ dashboard/user เปลี่ยน; `spec-trace.sh` = 26 เกณฑ์ครบ; viewports n/a (ยังไม่รัน dev server — structural+typecheck only); deviations none
